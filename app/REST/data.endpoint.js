...
const dataEndpoint = (router) => {
    ...
    router.get('/api/post/:id', async (request, response, next) => {
        response.status(200).send(posts.find(obj => obj.id === parseInt(request.params.id)));
    });
    ...
    router.put('/api/post', async (request, response, next) => {
        posts[posts.findIndex(obj => obj.id === parseInt(request.body.editPost.id))] = request.body.editPost;

        response.status(200).send(posts[posts.findIndex(obj => obj.id === parseInt(request.body.editPost.id))]);
    });
};
